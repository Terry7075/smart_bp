...
class ActivePrescriptionCard extends ConsumerWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Builder(builder: (_) {
              final morning =
                  _userTimesInRange(start: _morningStart, end: _noonStart);
              final noon =
                  _userTimesInRange(start: _noonStart, end: _eveningStart);
              final evening =
                  _userTimesInRange(start: _eveningStart, end: _bedtimeStart);
              final bedtime =
                  _userTimesInRange(start: _bedtimeStart, end: _morningStart);
              return Row(
                children: [
                  Expanded(
                    child: _SlotTile(
  ...
  Widget? _refillBanner(PrescriptionRecord record) {
    final message = switch (record.refillStatus) {
      RefillStatus.collecting =>
        '🛵 志工正在幫您代領下個月的藥，請安心等候！',
      RefillStatus.outOfStock =>
  ...
  void _openCheckin(BuildContext context) {
    ...
    context.push('/medication-checkin?prescriptionId=$enc');
  }
}

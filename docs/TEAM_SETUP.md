...
| 家屬 | `family` | `/family/home`（須綁定長輩 UUID） |

註冊畫面可勾選「我是社區志工」自動設 `volunteer`。

**家屬綁定**：家屬帳號 → 家屬關懷 → 輸入長輩的 `auth.users.id`（UUID）。需已執行 `graduation_enhancement_schema.sql`。
...
| 家屬綁定失敗 | 跑 `graduation_enhancement_schema.sql` |
